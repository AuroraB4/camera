<template>
  <div class="app">
    <div class="search-section">
      <div class="component-wrapperSearch">
        <div class="v-alignedSearch">
          <input class="input" id="term" type="text" v-model="inputTerm" placeholder="Enter a term...":class="{ 'warn': inputAlertVisible}" />

          <button type="button" @click="submitRequest('definition')" class="search-button">Definition suggestion</button>
          <button type="button" @click="submitRequest('reference')" class="search-button">Reference suggestion</button>
          <div id="alert" v-if="inputAlertVisible||eurovocAlertVisible">{{ alert }}</div>
        </div>

        <div class="v-aligned">
          <input class="datepicker" type="date" v-model="selectedDate"/>
          <div :class="{ 
            'inputEurovocContainer': true, 'active': inputEurovocs.length > 0 && eurovocInputFocused, 'warn': eurovocAlertVisible }">
            <input id="eurovocInput" type="text" v-model="inputEurovoc" placeholder="Enter an eurovoc..."   @focus="eurovocInputFocused = true"
            @blur="eurovocInputFocused = false"/> 
            <button id="add-button" @click="addEurovoc">+</button>
          </div>
          <div v-if="inputEurovocs.length > 0">
              <div class="eurovocs" v-for="(eurovoc, index) in inputEurovocs" :key="index">
                {{ eurovoc }} <button id="add-button" @click="removeEurovoc(index)">x</button>
              </div>
          </div>
        </div>
        <div class="v-aligned checkboxes">
          <div>
             <input type="radio" id="leosCh" name="repository" value="leos" checked />
             <label for="huey">LEOS</label>
          </div>
          <div>
            <input type="radio" id="normattivaCh" name="repository" value="normattiva"/>
            <label for="normattivaCh">Normattiva</label>
          </div>
          <div>
            <input type="radio" id="ddlCameraCh" name="repository" value="ddlCamera"/>
    ...
    <div class="results">
      <Request v-if="requestVisible" :term="requestTerm" :eurovocs="requestEurovocs" :date="requestDate"/>
...
<style>
  ...
  #term{
    padding: 7px;
    padding-right: 30px;
    background: white url("https://www.svgrepo.com/show/532555/search.svg") no-repeat calc(100% - 10px) center;
